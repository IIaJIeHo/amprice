...
app.controller('MainCtrl', ["$scope","$http","NgTableParams",function($scope,$http,NgTableParams) {
  var convert = { raw : 'Сырец',ball: 'Шары',index: 'Индексы',domenic:'Доминиканский',baltic:'Балтийский',ukranian:'Украинский',mexican:'Мексиканский',indonezian:'Индонезийский',dominic:'Dominican'};
  function call_data(){
    init_default_data();
  ...
  function call_version() {
    $http.get('http://ru.amberprice.net/wp-json/posts?type[]=tableme&filter[category_name]=version')
        .then(function(data){
            try{
  ...
  function make_things_done(main){
            ...
            $scope.store_indexes = $scope.bundle.filter(function(data){
                return data.amber_type == 'Индексы';
            }).sort(function(data1,data2){
                return data1.time - data2.time;
            });
  ...
  function first_call(length,callback){
    $http.get('http://ru.amberprice.net/wp-json/posts?filter[posts_per_page]=1&type[]=tableme&page='+(length+1))
        .then(function(data){
  ...
  function call_ajax_to_data() {
    $http.get('http://ru.amberprice.net/wp-json/posts?filter[posts_per_page]=100&type[]=tableme')
        .then(function(data){
  ...
  function call_ajax_to_first(posts_per_page) {
    $http.get('http://ru.amberprice.net/wp-json/posts?filter[posts_per_page]='+posts_per_page+'&type[]=tableme&filter[category_name]=first')
        .then(function(data){
            if (data.data.length != 0) {
                var main = postupdate(data);
                if (counter_main < 7){
  ...
  function call_ajax_to_every(posts_per_page) {
    $http.get('http://ru.amberprice.net/wp-json/posts?filter[posts_per_page]='+posts_per_page+'&type[]=tableme&filter[category_name]=every')
        .then(function(data){
            if (data.data.length != 0) {
                var main = postupdate(data);
                if (counter_main < 7){
  ...
  function call_ajax_to_add_data(posts_per_page,page) {
    $http.get('http://ru.amberprice.net/wp-json/posts?filter[posts_per_page]='+posts_per_page+'&type[]=tableme&page='+page)
        .then(function(data){
            if (data.data.length != 0) {
                var main = postupdate(data);
        ...
        $http.get('http://ru.amberprice.net/wp-json/posts?type[]=events&&filter[posts_per_page]=1000')
        .then(function(data){
        ...
        $scope.$watch('state',function(new_one,old_one){
            if (!$scope.multidata){
                $scope.options.noData = "Нету данных по выбранным категориям";
            }
            ...
            if ((new_one.amber_type != old_one.amber_type)||(new_one.amber_class != old_one.amber_class)){
                ...
                if ((new_one.amber_class == 'Индонезийский')&&(new_one.amber_type == 'Сырец')){
                    $scope.state.frakcii = '<50г.';
                }
                if ((new_one.amber_class == 'Доминиканский')&&(new_one.amber_type == 'Сырец')){
                    $scope.state.frakcii = '>100г.';
                }
                if ((new_one.amber_class != old_one.amber_class)&&(new_one.amber_class == "Доминиканский")){
                    if (!$scope.multilines) $scope.table_state = 'Янтарь сырец (голубой)';
                    $scope.state.sort = 'Голубой';
                    $scope.state.form = 'ААА';
                    if (new_one.amber_type == 'Сырец'){
                        $scope.state.frakcii = ">100г.";
                        $scope.state.sort = 'Голубой';
                    $scope.state.form = 'ААА';
                    } else if (new_one.amber_type == 'Шары'){
                        $scope.state.frakcii = "20+/5+";
                        $scope.state.sort = 'Голубой';
                    $scope.state.form = 'ААА';
                    }
                }
                if ((new_one.amber_class == 'Доминиканский')&&(new_one.amber_type == 'Шары')){
                    $scope.state.frakcii = '20+/5+';
                }
            ...
            if (new_one.amber_class != old_one.amber_class){
                if (new_one.amber_class == "Индонезийский"){
                    $scope.table_state = 'Янтарь сырец';
                } else if (new_one.amber_class == "Мексиканский"){
                    $scope.table_state = 'Шары';
                } else if (new_one.amber_class == "Доминиканский"){
                    if (!$scope.multilines) $scope.table_state = 'Янтарь сырец (голубой)';
                    $scope.state.sort = 'Голубой';
                    $scope.state.form = 'ААА';
                    if (new_one.amber_type == 'Сырец'){
                    ...
                    $scope.table_state = 'Янтарь сырец (Средние фракции)';
                }
                if (new_one.amber_type == 'Сырец'){
                    render_table_raw();
                } else if (new_one.amber_type == 'Шары'){
            ...
            if ($scope.state.amber_class != 'Доминиканский'){
                $scope.indostate.form = new_one.form.split('/')[1];
                if (['100г.','200г.','300г.','500г.','1000г.'].some(function (k) {
            ...
            if ($scope.data_bundle){
                try {
                    ...
                    //$scope.init_date = 'Сырец';
                    render_table_raw();
                    //$scope.init_date = 'Шары';
                    render_table_ball();
        ...
        $scope.$watch('showapp',function (new_value) {
            ...
            if (userAgent.match(/iPad/i) || userAgent.match(/iPhone/i)) {
                $scope.state.amber_class = 'Индонезийский';
                setTimeout(function (ar) {
                    $scope.state.amber_class = 'Балтийский';
                },50);
            }
        ...
        $scope.$watch('table_state',function(new_value){
            if ($scope.state&&!$scope.multilines){
                if (new_value.indexOf('Янтарь сырец') != -1){
                    $scope.state.amber_type = "Сырец";
                } else if (new_value.indexOf('Шары') != -1) {
                    $scope.state.amber_type = "Шары";
                } else {
                    $scope.state.amber_type = "Индексы";
                }
                if (new_value.indexOf('голубой') != -1){
                    $scope.state.sort = 'Голубой';
                } else if (new_value.indexOf('зеленый') != -1){
                    $scope.state.sort = 'Зеленый';
                } else if (new_value.indexOf('желтый') != -1){
                    $scope.state.sort = 'Желтый';
                } 
            }
        ...
        $scope.$watch('indostate',function(new_value){
            if ($scope.state.amber_class == 'Индонезийский') {
                $scope.state.form = 'Матовый/'+new_value.form;
            }
        ...
        function set_default_state_for_amber_type(amber_type){
            if (amber_type == 'Сырец'){
                $scope.state.frakcii = "20-50г.";
                $scope.state.sort = "ААА";
                $scope.state.form = "Матовый/Компактный";
                $scope.state.country = "Европа";
            } else if (amber_type == 'Шары'){
                $scope.state.frakcii = "12мм+/1г+";
                if ($scope.state.amber_class != 'Доминиканский'){
                    $scope.state.form = "Матовый/ААА";
                }
                
                $scope.state.country = "Европа";
            } else if (amber_type == 'Индексы'){
                $scope.state.index_type = "Общий";
            }
        }
        ...
        function init_default_data(){
            $scope.basestate = {
              type: "amber",
              country: "Европа",
              form: "Матовый/Компактный",
              frakcii: "20-50г.",
              sort: "АА",
              currency: "EUR",
              amber_class: "Балтийский",
              amber_type: "Сырец",
              index_type: "Общий",
              date: null,
        ...
        $scope.select_options = {
            ...
            amber_class: ["Балтийский","Индонезийский","Украинский","Мексиканский","Доминиканский"],
            amber_type: ["Сырец","Шары","Индексы"],
            index_type: ["Сырец","Шары","Индексы"],
            sort: ["ААА","А","В","МИКС","С"],
            form: ["Матовый/Компактный","Прозрачный/Компактный","Матовый/Плоский","Прозрачный/Плоский"],
            frakcii: ["1000г.","500г.","300г.","200г.","100г.","50-100г.","20-50г.","10-20г.","5-10г.","2.5-5г."],
            table: ["Янтарь сырец (Средние фракции)","Янтарь сырец (Крупные фракции)","Шары матовые","Шары прозрачные","Индексы"],
            graph_time: ["3 месяца","6 месяцев","1 год","Все данные"],
            graph_type: ["Абсолютные значения","Колебания цен"]
        }
        $scope.graph = {
        ...
        $scope.select_options.special = {
            amber_class: ["Балтийский","Индонезийский","Украинский","Доминиканский"],
            frakcii: ["50-100г.","20-50г.","10-20г.","5-10г.","2.5-5г."],
            frakcii2: [">1000г.",">500г.",">300г.",">200г.",">100г.","50-100г.","20-50г.","10-20г.","5-10г.","2.5-5г."],
        }

        $scope.select_options.indonezian = {
            frakcii: [">300г.","50-300г.","<50г."],
            sort: ["ААА","АА","А","В","С"],
            table: ["Янтарь сырец","Шары","Индексы"],
            form: ["ААА","АА","А","В","С"]
        }
        ...
        $scope.select_options.domenic = {
            frakcii: {
                raw:{
                    big: [">100г.","75-100г.","50-75г.","40-50г.","30-40г.","20-30г.","10-20г.","<10г."],
                    small: [">100г.","50-100г.","<50г."],
                },
                ball:["20+/5+","15+/2+","12+/1+","10+/0.5+","8+/0.25+","6+/0.1+","6-/0.1-"]
            },
            form: ["ААА","АА","А"],
            sort: ["Голубой","Зеленый","Желтый"],
            table: ["Янтарь сырец (голубой)","Янтарь сырец (зеленый)","Янтарь сырец (желтый)","Шары (голубой)","Шары (зеленый)","Шары (желтый)","Индексы"],
        }

        $scope.indostate = {};
        $scope.indostate.form = "ААА";
        $scope.indostate.frakcii = ">1000г.";


        $scope.select_options.mexican = {
            table: ["Шары","Индексы"],
            amber_type:["Шары","Индексы"],
            form: ["Матовый/ААА","Матовый/АА","Матовый/А","Матовый/В","Матовый/С"]
        }
        ...
        $scope.select_options.ball = {
            frakcii: ["40мм+/30г+",
                        ...
                        "6мм+/0.1г+",
                        "6мм-/0.1г-"],
            form: ["Матовый/ААА","Матовый/АА","Матовый/А","Матовый/В","Матовый/С","Прозрачный/ААА","Прозрачный/АА","Прозрачный/А","Прозрачный/В","Прозрачный/С"]
        };
        ...
        $scope.display = function (){
            ...
            if (data[0]){
                ...
                return {
                    ...
                    key: 'Cтоимость('+$scope.state.currency+')',
                    color: '#74aa9d',
                    strokeWidth: 2,
                    classed: 'dashed'
                };
        ...
        $scope.options = {
            chart: {
                ...
                noData:"Добавьте графики к сравнению",
        ...
        function render_table_ball(){
            base = [];
            if ($scope.state.date){
                $scope.current = get_current_bundle($scope.state.date.value,$scope.state.amber_class,'Шары');
            }
            if ($scope.current){
                if ($scope.state.amber_class == 'Доминиканский'){
        ...
        function render_table_raw(){
            base = [];
            if ($scope.state.date){
            $scope.current = get_current_bundle($scope.state.date.value,$scope.state.amber_class,'Сырец');
        }
            if ($scope.current) {
            if ($scope.state.amber_class == 'Индонезийский'){
            ...
            } else if ($scope.state.amber_class == 'Доминиканский'){
                ...
                $scope.select_options.frakcii.forEach(function(option){
                        var b = ['100г.','200г.','300г.','500г.','1000г.'].reverse();
                            ...
                            base.push({
                                frakcii:option,
                                form:"Прозрачный/Компактный"
                            });
                            if (biggy){
                                base.push({
                                    frakcii:option,
                                    form:"Матовый/Плоский"
                                });
                            }
        ...
        function get_element_table(sort_param){
            ...
            } else if (sort_param.type = 'Шары'){
                if ((sort_param.sort == 'Голубой')||(sort_param.sort == 'Зеленый')||(sort_param.sort == 'Желтый')){
                    criteria = function(value){
            ...
            if ($scope.init_date == 'Сырец'){
                result = $scope.price_table['Сырец'].filter(criteria)[0];
            } else if ($scope.init_date == 'Шары'){
                result = $scope.price_table['Шары'].filter(criteria)[0];
            } else {
        ...
        function criteria_func(){
           if ($scope.state.amber_type == 'Сырец'){
                ...
                if (($scope.state.sort == 'Голубой')||($scope.state.sort == 'Зеленый')||($scope.state.sort == 'Желтый')){
                    return function(value){
...
jQuery('.b-news .fusion-post-content-container p:last-of-type').each(function() {
  jQuery(this).text(jQuery(this).text()+'...');
    ...
    jQuery('.related-posts .title-heading-left').text('Другие публикации');
});
</script>